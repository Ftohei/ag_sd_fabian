...
public interface ArticleRepository extends CrudRepository<ArticleEntity, String>{
    ...
    @Query(value = "SELECT artikelId FROM artikel WHERE titel NOT LIKE '' AND length(text) > 500 AND datum = (SELECT max(datum) FROM artikel) AND (ressortId = 1 OR ressortId = 2 OR ressortId = 4)", nativeQuery = true)
